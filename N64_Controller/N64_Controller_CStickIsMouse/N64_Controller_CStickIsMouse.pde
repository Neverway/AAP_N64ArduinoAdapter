...
// Mouse Setup
int mouseSensitivity = 40; // Set c-mouse sensitvity
int horizontalOffset = 0; // Use this to fix left/right drifting with c-mouse
int verticalOffset = -1; // Use this to fix up/down drifting with c-mouse
int mx = 0, my = 0;
int sx, sy;
boolean mouse = false;
boolean mouseJoy = false;
...
void mouseToggle()
{
  // Toggle JoyStick mouse mode
  if(z && rr && ll && b)
  {
    mx = (screenWidth/2+horizontalOffset);
    my = (screenHeight/2+verticalOffset);
    mouse = false;
    mouseJoy = !mouseJoy;//have the start button enable/disable the mouse
  }
}
...
void serialEvent(Serial N64Connection)
{
  ...
  // Functions for mouse stuff
  mouseToggle();
  if(mouseJoy) // Check if C is in mouse mode
  {
    // Vertical
    if(cu)
    {
      my = (screenHeight/2+verticalOffset)-mouseSensitivity;
    }
    if(cd)
    {
      my = (screenHeight/2+verticalOffset)+mouseSensitivity;
    ...
    if(cl)
    {
      mx = (screenWidth/2+horizontalOffset)-mouseSensitivity;
    }
    if(cr)
    {
      mx = (screenWidth/2+horizontalOffset)+mouseSensitivity;
